...
Each version lives on its own branch.

```bash
# 26.1.x Fabric (main branch)
./gradlew build

# 26.1.x Forge (forge-26.1 branch)
cd forge && ./gradlew build

# 26.1.x NeoForge (neoforge-26.1 branch)
cd neoforge && ./gradlew build

# 1.21.4 Fabric (fabric-1.21.4 branch)
cd port-1.21.4/fabric && ./gradlew build

# 1.21.4 NeoForge (neoforge-1.21.4 branch)
cd port-1.21.4/neoforge && ./gradlew build

# 1.20.1 Fabric (fabric-1.20.1 branch)
cd port-1.20.1/fabric && ./gradlew build

# 1.20.1 Forge (forge-1.20.1 branch)
cd port-1.20.1/forge && ./gradlew build
```

Output goes to `build/libs/auto-replant-tree-2.1.0.jar`.

Note: For the 26.1.x Fabric build, ensure JAVA_HOME points to Java 25. For 1.20.1 builds, use Java 17.
